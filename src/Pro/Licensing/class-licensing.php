...
if ( ! class_exists( 'Licensing' ) ) {
	class Licensing {
		...
		public function __construct() {
			$this->website = 'https://burst-statistics.com';
		...
		private function get_css_margin(): int {
            ...
            // phpcs:ignore
			$css = file_get_contents( BURST_PATH . 'src/Pro/assets/css/general.css' );
		...
		private function fetch_license_status( string $action ): string {
			...
			$response   = wp_remote_post( $this->website, $api_params );
		...
		private function prepare_api_params( string $action, string $license ): array {
			...
			return apply_filters(
				...
				[
					...
					'headers'   => [
						'User-Agent' => 'Burst License Check',
					],
				]
			);
		}
		...
		private function handle_license_error( \WP_Error $response, int $attempts ): void {
			...
			if ( strpos( $message, '60' ) !== false && $attempts < 5 ) {
		...
		public function license_notices(): array {
			...
			$activations_left = get_site_option( 'burst_license_activations_left', 0 );
			...
			if ( $status === 'valid' || $status === 'inactive' || $status === 'deactivated' || $status === 'site_inactive' ) {
				...
				$messages[] = [
					...
					'url'     => $this->get_website_url(
						'account',
						[
							'burst_source'  => 'license_notices',
							'burst_content' => 'no-error-status',
						]
					),
				];
				...
				case 'invalid_item_id':
					$messages[] = [
						...
						'url'     => $this->get_website_url(
							...
							[
								'burst_source'  => 'license_notices',
								'burst_content' => 'item_name_mismatch',
							]
						),
					];
					break;
				case 'no_activations_left':
					...
					$messages[] = [
						...
						'url'     => $this->get_website_url(
							...
							[
								'burst_source'  => 'license_notices',
								'burst_content' => 'no_activations_left',
							]
						),
