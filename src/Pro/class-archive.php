...
class Archive {
	...
	private $archive_after_months;
	private $archive_option = false;
	private $rows_per_batch = 7500;
	/**
	 * Constructor
	 */
	...
	public function do_rest_action( array $output, string $action, array $data ): array {
		...
		if ( $action === 'get_archives' ) {
			...
			$this->update_option( 'archive_data', 'none' );
	...
	private function archive_upload_dir(): string {
		...
		if ( ! $folder ) {
			$folder = md5( uniqid( (string) wp_rand(), true ) );
	...
	private function map_dropped_column_value( string $column, string $value ): string {
		...
		return \Burst\burst_loader()->frontend->tracking->get_lookup_table_id_cached( $column, $value );
	}
	...
	private function sanitize_status( string $status ): string {
		return in_array( $status, [ 'archived', 'restored', 'archiving', 'archived', 'deleted' ], true ) ? $status : 'archived';
	}
