<?php
namespace Burst\TeamUpdraft\AutoInstaller;
 ...
 * Auto installer
 *
 * To be determined if this class will include all installable premium plugins, or only intstalls the premium version of itself.
 * If all plugins are to be included, TEAMUPDRAFT_AUTOINSTALLER could be used to prevent duplicates, or all prefixes can be changed per plugin.
 *
 * @version 2.0
 */
if ( ! class_exists( 'Auto_Installer' ) ) {
	class Auto_Installer {
		...
		public function __construct( string $caller_slug ) {
			// use generic prefix instead of Burst one.
            // phpcs:ignore
			define( 'TEAMUPDRAFT_AUTOINSTALLER', true );
		...
		public function init(): void {
			...
			add_action( 'wp_ajax_teamupdraft_autoinstaller_rest_api_fallback', [ $this, 'rest_api_fallback' ] );
		}
		...
		public function rest_api_fallback(): void {
			...
			$action = '';
			if ( preg_match( '#/teamupdraft/v1/auto_installer/([^?&/]+)#', $request_url, $matches ) ) {
				$action = $matches[1];
			}
			...
			$request = new \WP_REST_Request( 'GET', '/teamupdraft/v1/auto_installer/' . $action );
		...
		public function enqueue_assets( ?string $hook ): void {
            ...
            if ( $hook === 'plugins.php' && isset( $_GET['install_pro'] ) ) {
				...
				wp_enqueue_style( 'teamupdraft-autoinstaller-css', plugin_dir_url( __FILE__ ) . 'styles.min.css', [], $file_mtime_css );

				$file_mtime_script = filemtime( plugin_dir_path( __FILE__ ) . 'scripts.min.js' );
				wp_enqueue_script( 'teamupdraft-script', plugin_dir_url( __FILE__ ) . 'scripts.min.js', [ 'wp-api-fetch' ], $file_mtime_script, true );
				wp_localize_script(
					'teamupdraft-script',
					'teamupdraft_autoinstaller',
					[
						'steps'          => $this->steps,
						'rest_url'       => rest_url(),
						'token'          => wp_create_nonce( 'teamupdraft_autoinstaller_nonce' ),
						'finished_title' => __( 'Installation finished', 'burst-statistics' ),
						'admin_ajax_url' => add_query_arg( [ 'action' => 'teamupdraft_autoinstaller_rest_api_fallback' ], admin_url( 'admin-ajax.php' ) ),
					]
				);
			}
		}
		...
		public function print_install_modal(): void {
			...
			if ( is_admin() ) {
				...
				?>
				<div id="teamupdraft-step-template">
					<div class="teamupdraft-install-step {step}">
						<div class="teamupdraft-step-color">
							<div class="teamupdraft-grey teamupdraft-bullet"></div>
						</div>
						<div class="teamupdraft-step-text">
							<span>{doing}</span>
						</div>
					</div>
				</div>
				<div id="teamupdraft-start-button">
					<button class="button-primary teamupdraft-start"><?php esc_html_e( 'Start', 'burst-statistics' ); ?></button>
				</div>
				<div id="teamupdraft-plugin-suggestion-template">
					<div class="teamupdraft-recommended"><?php esc_html_e( 'Recommended by Burst Statistics', 'burst-statistics' ); ?></div>
					<div class="teamupdraft-plugin-suggestion">
						<div class="teamupdraft-icon"><img alt="suggested plugin icon" src="<?php echo esc_url( $this->get_suggested_plugin( 'icon' ) ); ?>"></div>
						<div class="teamupdraft-summary">
							<div class="teamupdraft-title"><?php echo esc_html( $this->get_suggested_plugin( 'title' ) ); ?></div>
							<div class="teamupdraft-description_short"><?php echo esc_html( $this->get_suggested_plugin( 'description' ) ); ?></div>
							<div class="teamupdraft-rating">
								<?php
								$installer   = new Installer( $this->caller_slug );
						...
						<div class="teamupdraft-install-button"><a class="button-secondary" <?php echo esc_attr( $this->get_suggested_plugin( 'disabled' ) ); ?> href="<?php echo esc_url( $this->get_suggested_plugin( 'search_url' ) ); ?>"><?php echo esc_html( $this->get_suggested_plugin( 'button_text' ) ); ?></a></div>
					</div>
				</div>
				<div class="teamupdraft-modal-transparent-background">
					<div class="teamupdraft-install-plugin-modal">
						<?php // translators: 1: Plugin name to be installed. ?>
						<h3 class="teamupdraft-initial"><?php printf( esc_html__( 'Ready to install %s!', 'burst-statistics' ), esc_html( $this->plugin_name ) ); ?></h3>
						<?php // translators: 1: Plugin name to be installed. ?>
						<h3 class="teamupdraft-running" style="display: none"><?php printf( esc_html__( 'Installing %s...', 'burst-statistics' ), esc_html( $this->plugin_name ) ); ?></h3>
						<h3 class="teamupdraft-done" style="display: none"><?php esc_html_e( 'Ready!', 'burst-statistics' ); ?></h3>
						<div class="teamupdraft-progress-bar-container">
							<div class="teamupdraft-progress teamupdraft-grey">
								<div class="teamupdraft-bar teamupdraft-green" style="width:0%"></div>
							</div>
						</div>
						<div class="teamupdraft-install-steps">

						</div>
						<div class="teamupdraft-footer">
							<a href="<?php echo esc_url( $dashboard_url ); ?>" role="button" class="button-primary teamupdraft-hidden teamupdraft-btn teamupdraft-visit-dashboard">
								<?php esc_html_e( 'Visit Dashboard', 'burst-statistics' ); ?>
							</a>
							<a href="<?php echo esc_url( $plugins_url ); ?>" role="button" class="button-primary teamupdraft-red teamupdraft-hidden teamupdraft-btn teamupdraft-cancel">
								<?php esc_html_e( 'Cancel', 'burst-statistics' ); ?>
							</a>
							<div class="teamupdraft-error-message teamupdraft-folder teamupdraft-package teamupdraft-install teamupdraft-activate teamupdraft-hidden">
								<span>
								<?php
							...
							<div class="teamupdraft-error-message teamupdraft-license teamupdraft-hidden"><span>
							<?php
							esc_html_e( 'An Error Occurred:', 'burst-statistics' );
							?>
								</span>&nbsp;
		...
		public function verify_nonce( string $nonce ): bool {
			return wp_verify_nonce( sanitize_text_field( wp_unslash( $nonce ) ), 'teamupdraft_autoinstaller_nonce' );
		}
		...
		public function check_destination_clear( \WP_REST_Request $request ): \WP_REST_Response {
			...
			if ( ! $error ) {
				...
				if ( $wp_filesystem->is_file( $file ) ) {
					$dir     = dirname( $file );
					$new_dir = $dir . '_' . time();
					set_transient( 'teamupdraft_upgrade_dir_' . $this->slug, $new_dir, WEEK_IN_SECONDS );
		...
		private function validate_license( string $license, int $item_id ): array {
			...
			if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
				...
				if ( false === $license_data->success ) {
					...
					$new_dir = get_transient( 'teamupdraft_upgrade_dir_' . $this->slug );
					if ( $new_dir ) {
