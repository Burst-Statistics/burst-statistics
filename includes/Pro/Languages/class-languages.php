...
class Languages {
	...
	public function manage_languages(): void {
		...
		if ( ! empty( $language_files ) ) {
			...
			foreach ( $language_files as $file_data ) {
				...
				// if we're here, and the language file is already installed, no need to proceed.
				if ( ! $this->language_is_installed( $target_locale ) ) {
	...
	public function get_supported_languages(): array {
		...
		// Get used language for each admin
		$admin_languages = [];
		foreach ( $admin_users as $admin_id ) {
			$admin_locale = get_user_meta( $admin_id, 'locale', true );

			// fallback to site locale.
			if ( empty( $admin_locale ) ) {
				$admin_locale = $site_locale;
			}

			$admin_languages[] = $admin_locale;
		}

		// Voeg admin talen toe aan de hoofdlijst
		$languages = array_merge( $languages, $admin_languages );
